...
def preprocess(data):
    ...
    tok = tokenizer(data['text'], padding='max_length', truncation=True)
...
def compute_metrics(eval_pred):
    ...
    return accuracy.compute(predictions=preds, references=labels) | precision.compute(predictions=preds,
                                                                                      references=labels,
                                                                                      average="macro", zero_division = 1) | recall.compute(
...
if __name__ == "__main__":
    ...
    model = BertForSequenceClassification.from_pretrained("bert-base-cased", num_labels=7).to("cuda")
